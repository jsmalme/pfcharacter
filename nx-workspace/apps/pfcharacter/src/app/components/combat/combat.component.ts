...
export class CombatComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.character$.pipe(first()).subscribe((char: Character) => {
      this.weaponForm = this.initWeaponForm();
      this.calculateAcDexMod(
        char.abilities.dex.useMod,
        char.equipment.ac_items
      );
      this.calculateArmorAndShield(char.equipment.ac_items);
      char.combat_info.getCombatInfoTotals(
        char.abilities,
        this.acDexScore,
        this.acArmor,
        this.acShield
    ...
    this.combat_infoForm.valueChanges
      .pipe(debounceTime(1000))
      .subscribe((info) => {
        if (!this.combat_infoForm?.valid) {
          return;
        }
        this.store.updateCombatInfo(
          info,
          this.acDexScore,
          this.acArmor,
          this.acShield
    ...
    this.breakpointObserver
      ...
      .subscribe((state: BreakpointState) => {
        state.matches
          ? (this.weaponObjectHeight = '15em')
          : (this.weaponObjectHeight = '20em');
      });
  }
    ...
    ac_items.forEach((item) => {
      if (item.equipped) {
        ...
        if (item.type == acTypeEnum.shield) {
          this.acShield < (item.bonus ?? 0)
            ? (this.acShield = item.bonus ?? 0)
            : this.acShield;
        } else {
          this.acArmor < (item.bonus ?? 0)
            ? (this.acArmor = item.bonus ?? 0)
            : this.acArmor;
        }
      }
